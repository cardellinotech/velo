...
interface WeekReviewDialogProps {
  ...
  onClose: () => void;
  weekStart: string;
  record: WeeklyGoalsRecord | null | undefined;
}
...
export function WeekReviewDialog({ open, onClose, weekStart, record }: WeekReviewDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setReviewText(record?.weekReview ?? "");
    }
  }, [open, record?.weekReview]);
    ...
    >
      <div className="relative w-full max-w-lg mx-4 bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800">
          <div>
            <h2 className="text-sm font-semibold text-white">Wochenrückblick</h2>
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          {goals.length === 0 && (
            <p className="text-xs text-slate-600 italic">Keine Wochenziele gesetzt.</p>
          )}
